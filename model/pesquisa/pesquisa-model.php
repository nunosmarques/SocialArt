<?php
class PesquisaModel extends Methods {
	...
	public function getProduct(){
		if($this->counter == 0){
			echo '<h4 class="h4center">Não existem items '.$this->getQuery().'!</4>';
		}else{
			//$results = $this->db->selectLimit("produto", true, $clause, "Like", 'OR', $this->getStart(), $this->getPageLimit(), "ProdutoID DESC");
			
			//"SELECT $field FROM $table $free WHERE $string AND $del $more";
			//"SELECT * FROM $table WHERE ".$string."  AND deleted = 'N' ORDER BY ".$orderby." LIMIT $start , $limit";
			
			$clause = array("NomeProduto" => "%".$this->getQuery()."%", "SmallDescription"=> "%".$this->getQuery()."%",  "Categoria"=> "%".$this->getQuery()."%");
			$field = "*";
			$join = "INNER JOIN categoria ON categoria.CategoriaID = produto.Categoria_CategoriaID";
			$results = $this->db->freeSelect("produto", $field, true, $clause, "LIKE", "OR", $join, "produto.deleted = 'N'", "ORDER BY produto.ProdutoID DESC LIMIT ".$this->getStart().", ".$this->getPageLimit()."");			
	...
	public function pagination(){
		echo '<a class="text25px" href="'.HOME_URI.'/pesquisa/index/pagina-1/'.$this->getQuery().'">&laquo;</a>
				<a class="text25px" href="'.HOME_URI.'/pesquisa/index/pagina-'.($this->getActualPage() > 1 ? ($this->getActualPage()-1) : 1).'/'.$this->getQuery().'">&lsaquo;</a>';
		for($i = 1; $i <= $this->getTotalPages(); $i++){		
					echo '<a href="'.HOME_URI.'/pesquisa/index/pagina-'.$i.'/'.$this->getQuery().'" class="page-num">'.$i.'</a>';
		}
		echo '<a class="text25px" href="'.HOME_URI.'/pesquisa/index/pagina-'.($this->getActualPage() < $this->getTotalPages() ? ($this->getActualPage()+1) : $this->getTotalPages()).'/'.$this->getQuery().'">&rsaquo;</a>
			  <a class="text25px" href="'.HOME_URI.'/pesquisa/index/pagina-'.$this->getTotalPages().'/'.$this->getQuery().'">&raquo;</a>';
	...
	public function countRows(){
		if(empty($this->getQuery())){
			...
			$clause = array("NomeProduto" => "%".$this->getQuery()."%", "SmallDescription"=> "%".$this->getQuery()."%",  "Categoria"=> "%".$this->getQuery()."%");
			$field = "ProdutoID, Categoria";
			$join = "INNER JOIN categoria ON categoria.CategoriaID = produto.Categoria_CategoriaID";
			return	count($this->db->freeSelect("produto", $field, true, $clause, "LIKE", "OR", $join, "produto.deleted = 'N'", "ORDER BY produto.dataAdicionado ASC"));
		}
	}
	...
	public function addItems(){
		...
		if(!$this->login->chkUserLogin()){  
			$this->form_data['Comprador'] = session_id(); 
		}else{
			$this->form_data['Comprador'] = $_SESSION["userdata"][0]['userid'];
		}
		...
		$this->form_data['dataAdicionado'] = $data;
		$result = $this->db->simpleInsert("carrinho",$this->form_data);
